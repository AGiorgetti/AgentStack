...
import { PackSelection } from "./core/types.js";
...
async function handleInit(): Promise<void> {
  ...
  const { plans, warnings, errors } = await planSelection(
    rootDir,
    templateDir,
    selection,
    options.managedBlocks,
    options.force,
  );
...
async function promptSelection(options: string[]): Promise<string[]> {
...
function resolveOptions(): { force: boolean; managedBlocks: boolean; dryRun: boolean } {
  return {
...
function reportPlanIssues(warnings: string[], errors: string[]): void {
...
function getPositionalVendor(): string | undefined {
  if (command !== "add") {
    return undefined;
  }

...
flowchart TD
  ...
  Lifecycle --> RulesModule["rules module"]
  ProtocolModule --> SourceAssets["assets/modules/protocol"]
  RulesModule --> RulesTemplate["assets/modules/rules/rules.md"]
  RulesModule --> AgentsMd["root AGENTS.md managed block"]
...
- **Rules module**: installs AgentStack's built-in rules template into a managed block at the top of root `AGENTS.md`.
...
The rules module lifecycle, managed-block behavior, and asset ownership live in [Rules Architecture](modules/rules/architecture.md).

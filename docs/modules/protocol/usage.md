...
This manual explains the implemented AgentStack Protocol workflows. For exact command flags and output contracts, use [Protocol Commands](commands.md) or `agentstack help ... --json`.

## Setup Workflow

```mermaid
flowchart LR
  Start["Choose tracker"] --> Setup["agentstack setup protocol"]
  Setup --> Assets["Deploy .agent-stack/modules/protocol"]
  Setup --> Skills["Deploy .agents/skills"]
  Setup --> AgentsMd["Merge AGENTS.md block"]
  Setup --> Manifest["Write .agent-stack/modules.json"]
...
`--github-repository` is required unless setup can infer the repository from the target repo's `origin` remote. `--provision-tracker` creates or updates default protocol labels for execution mode, readiness, type, state, claim, and priority.
...
Azure DevOps setup requires `--azdo-organization` and `--azdo-project`. The default Azure DevOps profile uses tags and built-in relation types. Provisioning verifies Azure CLI access and configured project reachability.
...
`.agent-stack/modules/protocol/policy/AGENT-POLICY.json` contains repository-local gates for autonomous agents. The CLI loads this file for tracker-backed commands and merges it over built-in defaults.
...
Worktree bootstrap uses `.agent-stack/modules/protocol/workspace.json` for optional defaults:
...
Agents should use `.agents/skills/agentstack-protocol-pr-complete/SKILL.md` when a human explicitly authorizes agent completion, and `.agents/skills/agentstack-protocol-pr-rework/SKILL.md` when PR review comments require more work.
...
The agent should use `agentstack-protocol-pr-complete`, re-check PR status and required checks, merge using normal PR tooling, then synchronize protocol state:
...
Today `release` means "remove the active claim marker" and writes a release comment. Teams that want a cleaner successful-completion audit trail should prefer a future dedicated `complete` command instead of overloading release.
...
The agent should use `agentstack-protocol-pr-rework`, move protocol state back to implementation, and record what it is doing:
...
If requested changes reveal a blocker rather than normal follow-up work, use `block` instead:
...
`doctor` validates the repository-local protocol module structure and rejects tracker configs that still contain placeholders such as `OWNER/REPO`.
...
node dist/agentstack.js setup protocol \
...
Create a smoke work item with protocol labels:
...
| --- | --- | --- |
| `AgentStack protocol module is not installed` | Protocol commands were run before `agentstack setup protocol`. | Install the protocol module or pass `--repo` pointing to an installed repository. |
...
| Placeholder config error | Tracker config still contains `OWNER/REPO`, `ORG`, or `PROJECT`. | Re-run `agentstack setup protocol ... --overwrite` with real tracker values. |

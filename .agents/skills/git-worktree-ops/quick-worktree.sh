...
Arguments:
  ...
  base-branch Base branch to branch from (overrides .agent-stack/workspace.json and git default branch)
EOF
}
...
read_workspace_config() {
  local key="$1"
  local config="${REPO_ROOT}/.agent-stack/workspace.json"
  if [[ ! -f "$config" ]]; then
    return 0
  fi
...
resolve_base_branch() {
  ...
  echo "Could not resolve base branch. Configure .agent-stack/workspace.json baseBranch or pass one explicitly." >&2
  exit 1
}
